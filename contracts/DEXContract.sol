...
contract Dex is ReentrancyGuard {
    ...
    event BuyPartialOrder(uint indexed _depositId, uint indexed _rate);
    ...
    function updateOrder(
        ...
        deposits[_orderId].expiryTime =
            deposits[_orderId].expiryTime +
            (_validityTime * 1 seconds);
    ...
    function claimOrder(uint _depositId) external noReentrant returns (uint) {
        ...
        uint localsellerAmount = depositObj.sellerAmount;
        uint localbuyerAmount = depositObj.buyerAmount;
    ...
    function buyPartialOrder(
        ...
        uint rate = (depositObj.sellerAmount * 10 ** 10) /
            depositObj.buyerAmount;
    ...
    function refundActiveDeposit(uint _depositId) external {
        ...
        require(
            depositObj.depositAddress == msg.sender,
            "UNAUTHORISED_ACCESSABILITY"
        );
    ...
    function gettokenbalance(
        IERC20 _token, //Contract address of T1
        address _address
    ...
    function getdeposites() public view returns (Deposit[] memory) {
        Deposit[] memory id = new Deposit[](deposits.length);
        for (uint i = 0; i < deposits.length; i++) {
            Deposit storage deposits1 = deposits[i];
            id[i] = deposits1;
        }
        return id;
    }
...
// contract address goerli network 0x3b04Bd5AEE04b1beE0bec34498eABaac7C3073f3
